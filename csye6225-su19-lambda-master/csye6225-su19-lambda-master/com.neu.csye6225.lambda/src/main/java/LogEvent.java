...
public class LogEvent implements RequestHandler<SNSEvent, Object> {
    ...
    public Object handleRequest(SNSEvent request, Context context) {
        ...
        long ExpirationTime = CurrentTime + 15*60;
        ...
        if (CheckToken(email)) {
            SendEmail(email, createItems(email, subject, CurrentTime, ExpirationTime),context);
            context.getLogger().log("Invoked Send email function");
        }
...
//              .withStringSet("Authors", new HashSet<String>(Arrays.asList("Author12", "Author22")))
    ...
    private static String SendEmail(String email,String token, Context context){
        String systemEmail = System.getenv("SYSTEM_EMAIL");
        String domainName = System.getenv("DOMAIN_NAME");
        final String FROM = systemEmail;
        final String SUBJECT = "Library Management Password Reset Email";
        final String HTMLBODY = "<h1>Your link for password reset -</h1>"
                + "<p>The password reset link: " + "<a href='https://aws.amazon.com/ses/'>" + domainName + "?email="
                + email + "&token=" + token + "</a>";
    ...
    private static boolean CheckToken(String email) {
        Table table = dynamoDB.getTable(tableName);
